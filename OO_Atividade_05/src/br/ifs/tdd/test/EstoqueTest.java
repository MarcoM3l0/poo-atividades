...
class EstoqueTest {
	...
	void ct33_adicaoDataValidadeVencida() {

		estoque.cadastrar(produtoValido());
		
		ValidacaoException ex = assertThrows(ValidacaoException.class, 
				() -> estoque.adicionar("1234567890123", 30, LocalDate.now().minusDays(1)));
	...
	void ct34_retiradaItemValido() {
		
		estoque.cadastrar(produtoValido());
		estoque.adicionar("1234567890123", 15, LocalDate.now().plusDays(30));
		estoque.retirar("1234567890123", 5);
		
		assertEquals(estoque.obterQuantidade("1234567890123"), 10);
	}
	
	
	@Test
	@DisplayName("CT35: Retirada priorizando lotes com validade mais próxima")
	void ct35_RetiradaPriorizada() {
		
		LocalDate validadeProxima = LocalDate.now().plusDays(10);
		LocalDate validadeDistante = LocalDate.now().plusDays(30);

		estoque.cadastrar(produtoValido());
		estoque.adicionar("1234567890123", 10, validadeProxima);
		estoque.adicionar("1234567890123", 20, validadeDistante);
		
		estoque.retirar("1234567890123", 15);
		List<Lote> lotesRestantes = estoque.listarLotes("1234567890123");

		int qntValidadeProxima = lotesRestantes.stream().filter(l -> l.getDataValidade().equals(validadeProxima)).mapToInt(Lote::getQuantidade).sum();
		int qntValidadeDistante = lotesRestantes.stream().filter(l -> l.getDataValidade().equals(validadeDistante)).mapToInt(Lote::getQuantidade).sum();
		
		assertEquals(0, qntValidadeProxima);
		assertEquals(15, qntValidadeDistante);
	}
	
	@Test
	@DisplayName("CT36: Retirada de itens utilizando identificador de produto nulo")
	void ct36_retiradaIdentificadorNulo() {
	...
	void ct37_retiradaProdutoNaoCadastrado() {
	...
	void ct38_retiradaQuantidadeNegativa() {
	...
	void ct39_retiradaQuantidadeMaior() {

		estoque.cadastrar(produtoValido());
		estoque.adicionar("1234567890123", 4, LocalDate.now().plusDays(30));
	...
	void ct40_obtencaoQuantidadeTotal() {
		
		estoque.cadastrar(produtoValido());
		estoque.cadastrar(new Produto("0987654321098", "Caneta preta", "Esferográfica preta", 2.10));
		
		estoque.adicionar("1234567890123", 15, LocalDate.now().plusDays(30));
		estoque.adicionar("0987654321098", 5, LocalDate.now().plusDays(30));
	...
	void ct401_obtencaoQuantidadeEspecifico() {
		...
		assertEquals(15, estoque.obterQuantidade("1234567890123"));
	}
	
	
	@Test
	@DisplayName("CT42: Obtenção da quantidade de produto não cadastrado")
	void ct42_obtencaoQuantidadeNaoCadastrado() {
		
		assertThrows(ProdutoNaoEncontradoException.class, 
				() -> estoque.obterQuantidade("1234567890123"));
	}
	
	
	@Test
	@DisplayName("CT43: Obtenção da quantidade utilizando identificador nulo")
	void ct43_obtencaoQuantidadeNula() {
    ...
    void ct44_relatorioTextual() {
        Produto produto1 = produtoValido();
        Produto produto2 = new Produto("0987654321098", "Caneta preta", "Esferográfica preta", 2.10);
        estoque.cadastrar(produto1);
        estoque.cadastrar(produto2);
        estoque.adicionar(produto1.getId(), 5, LocalDate.now().plusDays(10));
        estoque.adicionar(produto2.getId(), 3, LocalDate.now().plusDays(5));

        String relatorio = estoque.exibirRelatorio();
        
        assertTrue(relatorio.contains("ID"));
        assertTrue(relatorio.contains("Nome"));
        assertTrue(relatorio.contains(produto1.getId()));
        assertTrue(relatorio.contains(produto2.getId()));
        assertTrue(relatorio.matches("(?s).*Caneta.*Esferográfica.*2.50.*5.*"));
        assertTrue(relatorio.matches("(?s).*Caneta preta.*Esferográfica preta.*2.10.*3.*"));
    }
    ...
    void ct45_relatorioCSV() {
        ...
        estoque.adicionar(produto.getId(), 2, LocalDate.now().plusDays(7));

        String relatorioCsv = estoque.exibirRelatorioCSV();
        List<String> linhas = Arrays.asList(relatorioCsv.split("\n"));
        
        assertEquals("id,nome,descricao,preco,quantidade,validade", linhas.get(0));
        
        
        assertTrue(linhas.get(1).startsWith(produto.getId() + ","));
        assertTrue(linhas.get(1).contains(",2,"));
    }
    ...
    void ct46_relatorioJSON() {
        Produto produto = produtoValido();
        estoque.cadastrar(produto);
        LocalDate dataValidade = LocalDate.now().plusDays(15);
        estoque.adicionar(produto.getId(), 4, dataValidade);

        String relatorioJson = estoque.exibirRelatorioJSON();
        
        assertTrue(relatorioJson.startsWith("["));
        assertTrue(relatorioJson.endsWith("]"));
        
        assertTrue(relatorioJson.contains("\"id\":\"" + produto.getId() +"\""));
        assertTrue(relatorioJson.contains("\"nome\":\"Caneta\""));
        
        assertTrue(relatorioJson.contains("\"quantidade\":4"));
        assertTrue(relatorioJson.contains(dataValidade.toString()));
    }
}
